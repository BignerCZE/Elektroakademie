...
ROOT = Path.cwd()
SERVICES = ROOT / "courses" / "services.py"
TESTS = ROOT / "courses" / "tests" / "test_certificate_validity_timezone.py"
...
new = '''    issued_date = timezone.localtime(issued_at).date()
    valid_until = (
        issued_date
        + relativedelta(years=3)
        - timedelta(days=1)
...
if count != 1:
    print(
        "CHYBA: services.py neodpovídá očekávanému stavu. "
        f"Očekáván 1 blok výpočtu platnosti, nalezeno {count}. "
        "Nic nebylo změněno."
    )
    sys.exit(1)
...
test_source = '''from datetime import datetime, timezone as dt_timezone
...
backup = SERVICES.with_suffix(
    SERVICES.suffix + ".certificate-timezone.bak"
)
...
print("Oprava časového pásma platnosti certifikátu aplikována.")
print("Změněno: courses/services.py")
print("Přidáno: courses/tests/test_certificate_validity_timezone.py")
print("Modely nebyly změněny a nevzniká migrace.")
