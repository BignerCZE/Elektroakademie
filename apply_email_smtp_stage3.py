...
old_email_block = '''EMAIL_TRANSPORT = os.getenv(
    "EMAIL_TRANSPORT",
    "preview",
)
'''
...
if "# E-mailový transport a SMTP" in settings_text:
...
elif old_email_block in settings_text:
    ...
    raise SystemExit(
        "CHYBA: config/settings.py neobsahuje očekávaný původní "
        "EMAIL_TRANSPORT blok. Skript nic nezměnil."
    )
...
print("HOTOVO:")
print(f"  upraveno: {transport_path.relative_to(ROOT)}")
print(f"  upraveno: {settings_path.relative_to(ROOT)}")
print(f"  vytvořeno: {test_path.relative_to(ROOT)}")
print()
print("Nebyla změněna databázová schémata ani modely; migrace nejsou potřeba.")
print()
print("Doporučené testy:")
